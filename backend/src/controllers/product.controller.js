...
const createProductOnShopify = async (req, res) => {
 ...
 productCache.del("shopify_products");
...
const getAllShopifyProducts = async (req, res) => {
  ...
  try {
    let allProducts = productCache.get("shopify_products");
    ...
    if (!allProducts) {
      ...
      productCache.set("shopify_products", allProducts); // Set the cache with a TTL of 1 hour
    }
