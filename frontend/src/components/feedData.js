...
const getHeaders = (accessToken) => ({
      ...
      messages.push({
       ...
       ...staticFeedData,
         ...
         material:parentProduct.material,  
          ...
          other_product_image_locator_2:parentProduct.other_product_image_locator_2,
          other_product_image_locator_3:parentProduct.other_product_image_locator_3,
          other_product_image_locator_4:parentProduct.other_product_image_locator_4,
          other_product_image_locator_5:parentProduct.other_product_image_locator_5,
        }
      });
  ...
  console.log(feedData.messages[0].attributes)
      // 🛠 Amazon Feed Flow
      if (!accessToken || !feedData) {
        return res.status(400).json({ error: "Missing accessToken or feedData." });
      }
  ...
  async function getFeedStatus(accessToken, feedId) {
    const res = await axios.get(
      `${region}/feeds/2021-06-30/feeds/${feedId}`,
      { headers: getHeaders(accessToken) }
    );
    return res.data;
  }
  ...
  const checkFeedStatus = async (req, res) => {
    try {
      ...
      if (status === "DONE") {
        ...
        const { url } = await getFeedResultDocument(accessToken, resultFeedDocumentId);
      ...
      console.error("Error checking feed status:", err?.response?.data || err.message);
      res.status(500).json({ error: err?.response?.data || err.message });
    }
  };
  ...
  module.exports = {createFeedProduct,checkFeedStatus}